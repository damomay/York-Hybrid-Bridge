...
def friendly_reason(reason: str) -> str:
    ...
    if "direct lan state read" in lower:
        detail = text.split(":", 1)[-1].strip()
        return f"Direct LAN state read was interrupted: {detail}"
    if "connection refused" in lower: return "York module connection was refused"
    if "connect timeout" in lower or "timed out" in lower: return "York module did not respond before the timeout"
    if lower.startswith("poll:"):
        detail = re.sub(r"https?://\S+", "York module", text.split(":", 2)[-1].strip())
        return f"Temporary direct LAN polling failure: {detail}"
    return text
