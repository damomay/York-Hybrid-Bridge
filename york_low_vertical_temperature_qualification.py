"""Guarded one-shot qualification of York Heat/Low/Vertical temperature."""
...
CASES = {
    "25-to-24": TemperatureCase(
        ...
        confirmation_token=(
            "WRITE-QUALIFIED-LOW-VERTICAL-TEMPERATURE-25-TO-24-ONCE"
        ),
        source_reference=(
            "target frame from successful Relay v2 temperature transaction #1 "
            "(2026-07-30 18:14:55)"
        ),
        expected_sha256=(
    ...
    "24-to-25": TemperatureCase(
        name="24-to-25",
        before_temperature=24.0,
        target_temperature=25.0,
        command=bytes.fromhex(
            "BB0001031901004401063A00000000000000000000000000000000000000D8"
        ),
        confirmation_token=(
            "WRITE-QUALIFIED-LOW-VERTICAL-TEMPERATURE-24-TO-25-ONCE"
        ),
        source_reference=(
            "target frame from successful Relay v2 temperature transaction #2 "
            "(2026-07-30 18:15:32)"
        ),
        expected_sha256=(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate Alpha.42's captured Heat/Low/Vertical temperature frames "
            "or execute one guarded case once."
        )
    )
    ...
    for case in CASES.values():
        _print_case(case)
    print("Capture-qualified targets: 24 and 25 °C only")
    print("Captured shape: On / Heat / Fan Low / Swing Vertical / Display On")
    print("Encoding observed in both captures: setpoint byte = 31 - temperature")
    print("Automatic retries: 0")
    print("Automatic restore: disabled")
    print(
        "Normal MQTT Low/Vertical temperature path: unchanged; "
        "Relay v2 fallback remains active"
    )
    ...
    report = _write_report(
        ...
        case,
        {
            ...
            "command": {
                ...
                "exact_relay_capture_match": True,
