"""Guarded one-shot qualification of exact captured York mode commands."""
...
CASES = {
    "heat-to-cool": ModeCase(
        name="heat-to-cool",
        command=YORK_QUALIFICATION_POWER_ON_COOL,
        confirmation_token="WRITE-QUALIFIED-MODE-HEAT-TO-COOL-ONCE",
        before={"mode": "heat", **COMMON_STATE},
        after={"mode": "cool", **COMMON_STATE},
        source_reference=(
    ...
    "cool-to-heat": ModeCase(
        name="cool-to-heat",
        command=YORK_QUALIFICATION_POWER_ON_HEAT,
        confirmation_token="WRITE-QUALIFIED-MODE-COOL-TO-HEAT-ONCE",
        before={"mode": "cool", **COMMON_STATE},
        after={"mode": "heat", **COMMON_STATE},
        source_reference=(
...
def _validate_case(case: ModeCase) -> None:
...
def _state_label(state: dict[str, Any]) -> str:
    return (
        f"On / {state['mode'].title()} / {state['temperature']:g} °C / "
        f"Fan {state['fan'].title()} / Swing {state['swing'].title()}"
    )


def _print_case(case: ModeCase) -> None:
    print(f"Mode {case.name.replace('-', ' ').title()} case:")
...
def _write_report(
    output_dir: Path, case: ModeCase, payload: dict[str, Any]
) -> Path:
    output_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    version_tag = APP_VERSION.replace(".", "-")
    path = output_dir / f"{version_tag}-mode-{case.name}-{stamp}.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate Alpha.37's exact running Heat to Cool and Cool to Heat "
            "frames or execute one guarded case once."
        )
    )
    ...
    print(f"Climate Bridge {APP_VERSION} — Mode One-Shot Qualification")
    ...
    print("Normal MQTT mode path: unchanged; Relay v2 fallback remains active")
    ...
    if args.case is None:
        raise SystemExit(
            "SAFE STOP: --case heat-to-cool or --case cool-to-heat is required; "
            "no socket opened"
        )
    ...
    print(
        f"MODE {case.name.upper()} ONE-SHOT RESULT: "
    ...
    print(f"Before mode: {before['mode']}")
    print(f"After mode: {after['mode']}")
    print(f"Report: {report}")
