...
class RelayManager:
    """Handle HTTP communication with the York tablet relay."""
    ...
    def _request_json(
        ...
        **kwargs: Any,
        ...
        url = f"{self.config.relay_url}/{endpoint.lstrip('/')}"
        ...
        try:
            ...
            raise RelayError(
                f"Relay request timed out after "
                f"{self.config.relay_timeout:g} seconds: {endpoint}"
            ) from exc
        ...
        if not isinstance(payload, dict):
            raise RelayError(
                f"Tablet relay returned an unexpected JSON type for "
