...
def test_transport_display_names_are_operator_friendly(tmp_path):
    relay_config = load_config(Path("config.example.yml"))
    direct_config = _direct_config(tmp_path)
    assert RelayTransport(relay_config).display_name == "Relay (Legacy)"
    assert YorkDirectTransport(direct_config).display_name == "York Direct (Experimental)"


def test_york_components_are_split_by_responsibility(tmp_path):
    transport = YorkDirectTransport(_direct_config(tmp_path))
    assert isinstance(transport.session, YorkProtocolSession)
    assert isinstance(transport.session.connection, YorkConnection)
    assert isinstance(transport.session.decoder, YorkPacketDecoder)


def test_captured_bb_frame_can_be_staged_without_state_guessing(tmp_path):
    transport = YorkDirectTransport(_direct_config(tmp_path))
    captured = bytes.fromhex(
        "BB 01 00 03 0F 01 00 31 06 00 00 00 00 00 00 00 00 5F 00 00 DF"
    )
...
def test_non_bb_frame_is_rejected():
    ...
    with pytest.raises(YorkFrameError):
        decoder.parse_frame(bytes.fromhex("AA 00 00 00"))


def test_state_encoding_remains_safely_disabled(tmp_path):
    transport = YorkDirectTransport(_direct_config(tmp_path))
