from __future__ import annotations

import json
import sys
from pathlib import Path

import pytest

from adapters.york.broadlink import (
    AUTH_COMMAND,
    AUTH_REPLY,
    INITIAL_KEY,
    MAGIC,
    QUERY_COMMAND,
    QUERY_REPLY,
    BroadlinkYorkTemperatureWriteClient,
    _SessionEnvelope,
    _crypt,
    broadlink_checksum,
)
from adapters.york.decoder import YorkPacketDecoder
from adapters.york.errors import YorkProtocolError
from adapters.york.temperature_command import (
    build_qualified_temperature_command,
    validate_qualified_temperature_command,
)
import york_dynamic_temperature_qualification as dynamic_tool


def _heat_low_frame(temperature: float) -> bytes:
    whole = int(temperature)
    frame = bytearray.fromhex(
        "BB 01 00 03 0F 01 00 34 18 20 00 00 00 00 00 00 00 6E 00 00 00"
    )
    frame[8] = 0x10 | (whole - 16)
    frame[9] = 0x02 if temperature != whole else 0x00
    checksum = 0
    for value in frame[:-1]:
        checksum ^= value
    frame[-1] = checksum
    return bytes(frame)


BEFORE = _heat_low_frame(24.0)
AFTER = _heat_low_frame(23.5)
TARGET_COMMAND = bytes.fromhex(
    "BB 00 01 03 19 01 00 44 01 08 02 02 00 00 00 00 00 00 00 00 "
    "00 00 00 00 00 00 00 00 00 00 EC"
)
SESSION_KEY = bytes.fromhex("00112233445566778899AABBCCDDEEFF")
DEVICE_ID = 0x12345678


def _reply(request: bytes, command: int, clear: bytes, key: bytes) -> bytes:
    packet = bytearray(0x38)
    packet[:8] = MAGIC
    packet[0x26] = command
    packet[0x28:0x2A] = request[0x28:0x2A]
    padded = clear + bytes((-len(clear)) % 16)
    packet[0x34:0x36] = broadlink_checksum(padded).to_bytes(2, "little")
    packet.extend(_crypt(key, padded, encrypt=True))
    packet[0x20:0x22] = broadlink_checksum(packet).to_bytes(2, "little")
    return bytes(packet)


class FakeSocket:
    def __init__(self, *args, **kwargs) -> None:
        self.sent: list[bytes] = []
        self.timeout = 0.0
        self.query_count = 0

    def __enter__(self):
        return self

    def __exit__(self, *args):
        return False

    def settimeout(self, value: float) -> None:
        self.timeout = value

    def sendto(self, packet: bytes, endpoint: tuple[str, int]) -> int:
        self.sent.append(packet)
        self.endpoint = endpoint
        return len(packet)

    def recvfrom(self, size: int):
        request = self.sent[-1]
        command = request[0x26]
        if command == AUTH_COMMAND:
            clear = DEVICE_ID.to_bytes(4, "little") + SESSION_KEY + bytes(60)
            response = _reply(request, AUTH_REPLY, clear, INITIAL_KEY)
        else:
            clear_request = _crypt(SESSION_KEY, request[0x38:], encrypt=False)
            length = int.from_bytes(clear_request[:2], "little")
            inner = clear_request[2 : 2 + length]
            if self.query_count == 0:
                assert inner != TARGET_COMMAND
                frame = BEFORE
            elif self.query_count == 1:
                assert inner == TARGET_COMMAND
                frame = AFTER
            else:
                assert inner != TARGET_COMMAND
                frame = AFTER
            self.query_count += 1
            clear = len(frame).to_bytes(2, "little") + frame
            response = _reply(request, QUERY_REPLY, clear, SESSION_KEY)
        return response, self.endpoint


@pytest.mark.parametrize(
    ("mode", "temperature", "expected_hex"),
    [
        (
            "cool",
            25.0,
            "BB0001031901004403060200000000000000000000000000000000000000E2",
        ),
        (
            "heat",
            23.0,
            "BB0001031901004401080200000000000000000000000000000000000000EE",
        ),
        (
            "heat",
            23.5,
            "BB0001031901004401080202000000000000000000000000000000000000EC",
        ),
        (
            "heat",
            24.0,
            "BB0001031901004401070200000000000000000000000000000000000000E1",
        ),
    ],
)
def test_generator_matches_independent_relay_captures(
    mode: str, temperature: float, expected_hex: str
):
    assert build_qualified_temperature_command(mode, temperature).hex().upper() == (
        expected_hex
    )


@pytest.mark.parametrize("temperature", [16.0, 16.5, 22.0, 23.5, 29.5, 30.0])
@pytest.mark.parametrize("mode", ["cool", "heat"])
def test_generated_range_is_canonical(mode: str, temperature: float):
    frame = build_qualified_temperature_command(mode, temperature)
    assert validate_qualified_temperature_command(frame) == (mode, temperature)
    assert len(frame) == 31
    checksum = 0
    for value in frame:
        checksum ^= value
    assert checksum == 0


@pytest.mark.parametrize("temperature", [15.5, 30.5, 23.25])
def test_generator_rejects_unqualified_temperature(temperature: float):
    with pytest.raises(YorkProtocolError):
        build_qualified_temperature_command("heat", temperature)


def test_generator_rejects_unqualified_mode():
    with pytest.raises(YorkProtocolError, match="only heat and cool"):
        build_qualified_temperature_command("auto", 23.0)


def test_validator_rejects_checksum_correct_noncanonical_frame():
    modified = bytearray(TARGET_COMMAND)
    modified[12] = 0x01
    modified[-1] ^= 0x01
    with pytest.raises(YorkProtocolError, match="non-canonical"):
        validate_qualified_temperature_command(bytes(modified))


def test_evidence_matches_generated_target():
    evidence = json.loads(
        (
            Path(__file__).parent
            / "protocols/york/qualification/alpha27_dynamic_temperature_command.json"
        ).read_text(encoding="utf-8")
    )
    assert bytes.fromhex(evidence["qualification_target"]["frame_hex"]) == (
        build_qualified_temperature_command("heat", 23.5)
    )
    assert [source["relay_transaction_id"] for source in evidence["sources"]] == [
        22,
        23,
    ]
    assert all(source["relay_verification_success"] for source in evidence["sources"])


def test_exact_pre_read_generated_write_post_read_sequence():
    sock = FakeSocket()
    decoder = YorkPacketDecoder()
    client = BroadlinkYorkTemperatureWriteClient(
        "192.0.2.27",
        80,
        "02:00:00:00:00:27",
        3,
        "heat",
        23.5,
        socket_factory=lambda *args: sock,
    )

    def approve(frame: bytes) -> None:
        state = decoder.decode_state(frame)
        assert state.power is True
        assert state.mode == "heat"
        assert state.temperature == 24.0
        assert state.fan == "low"
        assert state.swing == "off"

    result = client.execute(approve, post_write_delay_seconds=0)
    assert result.send_count == 4
    assert decoder.decode_state(result.after_frame).temperature == 23.5
    assert [packet[0x26] for packet in sock.sent] == [
        AUTH_COMMAND,
        QUERY_COMMAND,
        QUERY_COMMAND,
        QUERY_COMMAND,
    ]


def test_precondition_failure_sends_no_dynamic_write():
    sock = FakeSocket()
    client = BroadlinkYorkTemperatureWriteClient(
        "192.0.2.27",
        80,
        "02:00:00:00:00:27",
        3,
        "heat",
        23.5,
        socket_factory=lambda *args: sock,
    )

    def reject(frame: bytes) -> None:
        raise RuntimeError("state mismatch")

    with pytest.raises(RuntimeError, match="state mismatch"):
        client.execute(reject, post_write_delay_seconds=0)
    assert client.last_send_count == 2
    assert len(sock.sent) == 2


def test_session_rejects_arbitrary_frame():
    session = _SessionEnvelope(
        mac=bytes.fromhex("020000000027"),
        counter=0x8000,
        device_id=DEVICE_ID,
        key=SESSION_KEY,
    )
    with pytest.raises(YorkProtocolError):
        session.build_temperature_write_packet(bytes.fromhex("BB000104020100BD"))


def test_dynamic_tool_is_not_wired_into_normal_control():
    root = Path(__file__).parent
    for name in ("bridge.py", "mqtt_manager.py", "transport/factory.py"):
        source = (root / name).read_text(encoding="utf-8")
        assert "york_dynamic_temperature_qualification" not in source
        assert "BroadlinkYorkTemperatureWriteClient" not in source


def test_normal_direct_transport_remains_read_only():
    source = (
        Path(__file__).with_name("transport") / "york_direct_transport.py"
    ).read_text(encoding="utf-8")
    assert "direct LAN integration is read-only" in source


def test_validation_mode_opens_no_socket(tmp_path: Path, monkeypatch, capsys):
    config = tmp_path / "config.yml"
    config.write_text(
        Path("config.example.yml")
        .read_text(encoding="utf-8")
        .replace("direct_read:\n  enabled: false", "direct_read:\n  enabled: true"),
        encoding="utf-8",
    )

    def forbidden_socket(*_args, **_kwargs):
        raise AssertionError("validation mode opened a socket")

    monkeypatch.setattr("adapters.york.broadlink.socket.socket", forbidden_socket)
    monkeypatch.setattr(sys, "argv", ["dynamic-tool", str(config)])
    assert dynamic_tool.main() == 0
    output = capsys.readouterr().out
    assert "Captured Relay v2 match: exact (31/31 bytes)" in output
    assert "no socket opened and no packet transmitted" in output
