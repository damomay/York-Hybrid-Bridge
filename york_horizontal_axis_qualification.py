#!/usr/bin/env python3
"""Guarded one-shot qualification of York's independent Horizontal axis."""
...
CASES = {
    "vertical-to-both": SwingCase(
        ...
        target_swing="both",
        command=YORK_QUALIFICATION_HEAT_LOW_BOTH_21_5,
        confirmation_token="ENABLE-HORIZONTAL-AXIS-WITH-VERTICAL-21-5-ONCE",
        evidence=(
            "Relay v2 captured the exact 31-byte Both command at Heat/21.5/"
            "Low; the handheld remote independently toggles Vertical and "
            "Horizontal axes, so this case preserves Vertical while adding "
            "the command-side Horizontal flag 0x08"
        ),
        expected_sha256=(
            "84709fa013aa3b1f1caddc653fb9a5b0cd18d5e1ed08a49ba622d99cb2c4e4ae"
        ),
    ),
    "both-to-vertical": SwingCase(
        ...
        command=YORK_QUALIFICATION_HEAT_LOW_VERTICAL_21_5,
        confirmation_token="DISABLE-HORIZONTAL-AXIS-KEEP-VERTICAL-21-5-ONCE",
        evidence=(
            "Alpha.48 physically qualified the parameterised Heat/Low/"
            "Vertical command shape at 21.5 °C; this removes only the "
            "captured Horizontal 0x08 flag"
        ),
        expected_sha256=(
            "abe125694bbd2be3c58ac6e834269e7a4ea1275afb8d729cb6c04335da8b9bbb"
        ),
    ),
}
...
def _write_report(
    ...
    path = output_dir / f"alpha50-horizontal-axis-{case.name}-{stamp}.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate Alpha.50 independent Horizontal-axis frames offline or execute "
    ...
    print(f"Climate Bridge {APP_VERSION} — Independent Horizontal Axis Qualification")
