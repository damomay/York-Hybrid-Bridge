...
from york_horizontal_axis_qualification import CASES, main
...
def _state_frame(state: dict[str, object]) -> bytes:
    mode = {"cool": 0x01, "heat": 0x04}[str(state["mode"])]
...
def test_exact_independent_axis_and_return_frames_are_locked():
    assert YORK_QUALIFICATION_HEAT_LOW_BOTH_21_5.hex().upper() == (
        "BB00010319010044010A3A0A000000000000000000000000000000000000DE"
    )
    assert YORK_QUALIFICATION_HEAT_LOW_VERTICAL_21_5.hex().upper() == (
        "BB00010319010044010A3A02000000000000000000000000000000000000D6"
    )
    assert YORK_QUALIFICATION_HEAT_LOW_BOTH_21_5[10] == 0x3A
    assert YORK_QUALIFICATION_HEAT_LOW_BOTH_21_5[11] == 0x0A
    assert YORK_QUALIFICATION_HEAT_LOW_VERTICAL_21_5[10] == 0x3A
    assert YORK_QUALIFICATION_HEAT_LOW_VERTICAL_21_5[11] == 0x02
