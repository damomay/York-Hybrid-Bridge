...
class DiagnosticsManager:
    ...
    _stability_events: deque[bool] = field(default_factory=lambda: deque(maxlen=100), repr=False)

    def __post_init__(self) -> None:
        self._stability_events = deque(maxlen=max(10, int(self.stability_window_size)))
    ...
    def average_command_time_ms(self) -> int:
        return 0 if self.successful_timed_commands == 0 else round(self.command_duration_total_ms / self.successful_timed_commands)

    @property
    def command_success_rate(self) -> float:
        applicable = self.command_count - self.command_deferred_count - self.command_not_applicable_count
        if applicable <= 0:
            return 100.0
    ...
    def poll_success_rate(self) -> float:
        ...
        return 100.0 if attempts == 0 else round((self.poll_count / attempts) * 100, 1)

    @property
    def recovery_success_rate(self) -> float:
        attempts = self.recovery_count + self.recovery_failure_count
        return 100.0 if attempts == 0 else round((self.recovery_count / attempts) * 100, 1)

    def record_stability_event(self, success: bool) -> None:
        previous = self.stability_score
        self._stability_events.append(bool(success))
        self.stability_score = round((sum(self._stability_events) / len(self._stability_events)) * 100, 1)
        if self.stability_score >= 99.0:
            self.stability_status = "excellent"
        elif self.stability_score >= 95.0:
            self.stability_status = "good"
        elif self.stability_score >= 85.0:
            self.stability_status = "warning"
        else:
            self.stability_status = "poor"
        delta = self.stability_score - previous
        if delta >= 0.2:
    ...
    def record_command_duration(self, duration_ms: Any) -> None:
        ...
        self.fastest_command_time_ms = duration if self.fastest_command_time_ms == 0 else min(self.fastest_command_time_ms, duration)
        self.slowest_command_time_ms = max(self.slowest_command_time_ms, duration)
    ...
    def record_poll_duration(self, duration_ms: float) -> None:
        ...
        self.timed_polls += 1
        self.average_poll_time_ms = round(self.poll_duration_total_ms / self.timed_polls)
        self.fastest_poll_time_ms = duration if self.fastest_poll_time_ms == 0 else min(self.fastest_poll_time_ms, duration)
        self.slowest_poll_time_ms = max(self.slowest_poll_time_ms, duration)

    def update_narrative(self) -> None:
        uptime_seconds = int(time.monotonic() - self.started_monotonic)
        self.uptime_text = format_duration(uptime_seconds)
        ...
        if self.health_status == "excellent" and mqtt_ok and relay_ok:
            self.bridge_summary = f"Healthy • MQTT ✓ • Relay ✓ • {self.last_poll_time_ms} ms"
        elif self.health_status in {"critical", "warning"}:
            self.bridge_summary = f"{self.health_status.title()} • {self.health_reason}"
        else:
            self.bridge_summary = f"{self.health_status.title()} • MQTT {self.mqtt_status} • Relay {self.bridge_status}"

        if self.health_status in {"critical", "warning"}:
            self.health_advisor = self.health_reason
        elif self.stability_trend == "declining":
            self.health_advisor = "Stability is declining. Monitor the tablet relay and Wi-Fi connection."
        elif self.recovery_count > 0 and self.stability_score < 99:
            self.health_advisor = f"Recovered automatically. Stability is {self.stability_trend}."
        elif self.average_poll_time_ms >= 2000:
            self.health_advisor = "Communication is available, but relay response time is elevated."
        elif self.recovery_count > 0:
            self.health_advisor = f"Operating normally. Last recovery: {self.last_recovery_age.lower()}."
        else:
            self.health_advisor = f"Operating normally. Stability is {self.stability_trend}."
    ...
    def publish_metrics(self) -> None:
        ...
        values = {
            "bridge_version": self.app_version,
            "bridge_uptime": int(time.monotonic() - self.started_monotonic),
    ...
    def publish_command(self, *, command_json: str, result: str, transaction: dict[str, Any] | None = None) -> None:
        self.publish("last_command", command_json)
        self.publish("last_command_result", result)
        level = "INFO" if result in {"running", "success", "deferred", "not_applicable"} else "ERROR"
        self.record_event(level, f"Command {result}: {command_json}")
        if transaction:
            self.publish("last_transaction_id", transaction.get("transaction_id", ""))
