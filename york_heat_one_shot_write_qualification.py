"""Guarded Heat-mode one-shot direct-write qualification."""
...
APP_VERSION = "1.0.0-alpha.26"
CONFIRMATION_TOKEN = "WRITE-HEAT-23-TO-24-ONCE"
COMMAND_SHA256 = "c55bd6793f3c6a84934195822cec2e5834fc0fa4429252e13a17659cbc770925"
...
EXPECTED_AFTER = {**EXPECTED_BEFORE, "temperature": 24.0}
...
def _write_report(output_dir: Path, payload: dict[str, Any]) -> Path:
    ...
    path = output_dir / f"alpha26-heat-one-shot-{stamp}.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate or execute Alpha.26's captured Heat command once."
    )
    ...
    print(f"Climate Bridge {APP_VERSION} — Heat One-Shot Direct-Write Qualification")
    config = load_config(Path(args.config))
    decoder = YorkPacketDecoder()
    command_hash = hashlib.sha256(YORK_QUALIFICATION_HEAT_23_TO_24).hexdigest()
    ...
    print("Required live state: On / Heat / 23 °C / Fan Low / Swing Off")
    print("Qualified result: On / Heat / 24 °C / Fan Low / Swing Off")
    ...
    report = _write_report(
        Path(args.output_dir),
        {
            ...
            "status": status,
            "command": {
                "source": "York TFIAC Relay v2 transaction 25",
                "frame_hex": YORK_QUALIFICATION_HEAT_23_TO_24.hex(" ").upper(),
                "sha256": command_hash,
