...
from test_sprint_3124_heat_horizontal_axis_qualification import _state_frame
...
def _manager(before_swing, target_swing, *, live_before=None, live_after=None):
    ...
    def axis_factory(host, port, mac, timeout, requested_command):
    ...
    return DirectSwingManager(config, axis_client_factory=axis_factory), sock
...
def test_normal_control_uses_exact_alpha53_frame_once(
    before_swing, target_swing, command
):
    ...
    assert response["mode"] == "heat"
    assert response["temperature"] == 21.5
    assert response["fan"] == "low"
    assert response["_transaction"]["qualified_path"] == (
        "heat_21_5_low_horizontal_only_axis"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("horizontal", "vertical", {}),
        ("vertical", "horizontal", {}),
        ("off", "horizontal", {"mode": "dry"}),
        ("off", "horizontal", {"temperature": 21.0}),
        ("off", "horizontal", {"fan": "high"}),
        ("off", "horizontal", {"display": False}),
    ],
)
def test_unqualified_shape_stops_before_axis_client_or_socket(
    before_swing, target_swing, changes
):
    config = load_config(Path("config.example.yml"))

    def forbidden_client(*_args, **_kwargs):
        raise AssertionError("axis client created for unqualified state")
...
def test_live_pre_read_mismatch_stops_before_write():
    manager, sock = _manager(
        "off", "horizontal", live_before=_state("horizontal")
    )
    with pytest.raises(DirectSwingSafeStop, match="direct pre-read differs"):
        manager.command("horizontal", _state("off"))
...
def test_post_read_mismatch_is_not_success():
    manager, sock = _manager(
        "off", "horizontal", live_after=_state("off")
    )
...
def test_alpha53_qualification_tool_is_not_executable_in_container():
